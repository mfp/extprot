...
let pp_list f pp l =
  let pr_elems = fprintfs ";@ " f in
    fprintf pp "[@[ %a @]]" pr_elems l

let pp_array f pp l =
  let pr_elems = fprintfs ";@ " f in
    fprintf pp "[|@[ %a @]|]" pr_elems (Array.to_list l)

let pp_struct fields pp t =
  let rec pr_fields pp = function
      [] -> ()
    | [name, f] -> fprintf pp "@[<2>%s@ =@ %a@]" name f t
    | (name, f)::l -> fprintf pp "@[<2>%s@ =@ %a@];@ %a" name f t pr_fields l
  in
    fprintf pp "{ @[%a@] }" pr_fields fields
...
let pp_tuple2 ?constr f1 f2 pp (a, b) =
  fprintf pp "%s(@[%a,@ %a@])" (constr_string constr) f1 a f2 b
